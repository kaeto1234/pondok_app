    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="font-medium">{{ $pendaftaran->tahunAjaran->nama_tahun ?? '-' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-medium">{{ $pendaftaran->tempat_lahir ?? '-' }},
                            {{ $pendaftaran->tanggal_lahir ? $pendaftaran->tanggal_lahir->format('d M Y') : '-' }}</p>
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">Nama Ayah</p>
                        <p class="font-medium">{{ $pendaftaran->nama_ayah ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">Pekerjaan Ayah</p>
                        <p class="font-medium">{{ $pendaftaran->pekerjaan_ayah ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">Nama Ibu</p>
                        <p class="font-medium">{{ $pendaftaran->nama_ibu ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">Pekerjaan Ibu</p>
                        <p class="font-medium">{{ $pendaftaran->pekerjaan_ibu ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">No. Telepon</p>
                        <p class="font-medium">{{ $pendaftaran->telepon_orang_tua ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider">Email Wali</p>
                        <p class="font-medium">
                            @if ($pendaftaran->email)
                                {{ $pendaftaran->email }}
                            @else
                                <span class="text-red-500 font-semibold">⚠ Belum diisi – verifikasi akan gagal!</span>
                            @endif
                        </p>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="mb-6">
                    ...
                    @elseif($pendaftaran->status == 'diverifikasi')
                        <span
                            class="px-3 py-1 text-sm font-bold bg-green-100 text-green-700 rounded-full">DIVERIFIKASI</span>
                    @else
                ...
                @if ($pendaftaran->status == 'pending')
                    <div class="space-y-4">
                        <form action="{{ route('admin.ppdb.verify', $pendaftaran->id) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Catatan Verifikasi
                                    (Opsional)</label>
                                <textarea name="catatan" rows="3" class="w-full border rounded-lg p-2 text-sm"
                                    placeholder="Contoh: Berkas sudah lengkap dan sesuai."></textarea>
                            </div>
                            <button type="submit"
                                onclick="return confirm('Apakah Anda yakin ingin memverifikasi data ini? Santri baru akan otomatis terdaftar.')"
                        ...
                        <form action="{{ route('admin.ppdb.reject', $pendaftaran->id) }}" method="POST">
                            ...
                            <div id="reject-form" class="mt-4 hidden">
                                <div class="mb-3">
                                    ...
                                    <textarea name="catatan" required rows="3" class="w-full border rounded-lg p-2 text-sm"
                                        placeholder="Contoh: Berkas foto tidak jelas."></textarea>
                                </div>
                ...
                @else
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <p class="text-xs text-gray-400 mt-2">Diverifikasi pada:
                            {{ $pendaftaran->diverifikasi_pada ? $pendaftaran->diverifikasi_pada->format('d M Y H:i') : '-' }}
                        </p>
                    </div>
                @endif
            </div>
